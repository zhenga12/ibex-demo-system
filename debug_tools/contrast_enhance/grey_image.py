...
SCALE_FACTOR = 7 #set to 7 to get close to 640x... #22 #no issues on simulation when set to 64
...
if USE_YUV:
    yuv_image = cv.cvtColor(base_img, cv.COLOR_BGR2YUV)
    greyscale = yuv_image[:,:,0] #Y (luma) is index 0
...
if USE_YUV:
    yuv_image[:,:,0] = enhanced #overwrite Y channel with contrast enhanced values
    enhanced = cv.cvtColor(yuv_image, cv.COLOR_YUV2BGR)
cv.imwrite("enhance.png", enhanced)

if WRITE_TO_FILE:
    to_text(greyscale, "greyscale.txt")
    if not USE_YUV:
        to_text(enhanced, "enhanced.txt")
        to_text(enhanced, "enhanced_hex.txt", "hex")
