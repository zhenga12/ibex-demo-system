...
//result should be a complex, not int
cmplx_t cmplx_abs_sq(cmplx_t c) {
  cmplx_t res = {0};
  res.real = fp_mul_clamp(c.real, c.real) + fp_mul_clamp(c.imag, c.imag);
  return res;
}
...
int run_abs_sq_test(cmplx_t c, int dump_results) {
  ...
  int32_t result_soft;
  result_soft = pack_cmplx(cmplx_abs_sq(c));
  ...
  if (dump_results) {
    ...
    puts("Result (Packed) (from soft cmplx)\n");
    puthex(result_soft);
    puts("\n");

    puts("Result (Packed) (from hard cmplx)\n");
    puthex(result_hard);
    puts("\n");
  }
